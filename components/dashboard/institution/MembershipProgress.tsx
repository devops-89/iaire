...
const MembershipProgress: React.FC<MembershipProgressProps> = ({
  ...
  const tiers = [
    ...
    { 
        ...
        color: "#3B82F6",
        reqs: { educators: 2, publications: 5 } 
    },
    { 
        level: MEMBERSHIP_LEVEL.FELLOW, 
        icon: <Gavel fontSize="small" />, 
        color: COLORS.PRIMARY_NAVY,
        reqs: { educators: 2, publications: 10, approved: true } 
    },
  ];
  ...
  if (nextTier) {
    ...
    if (reqs.educators !== undefined) {
      total++;
      if (educators >= reqs.educators) completed++;
    }
    ...
    >
      ...
      {nextTier && (
        <Stack direction="row" spacing={2} sx={{ mt: 2 }}>
            {nextTier.reqs?.educators !== undefined && (
                <Box sx={{ textAlign: 'center', flex: 1, p: 1.5, bgcolor: '#fcfcfc', borderRadius: '12px', border: '1px solid #f5f5f5' }}>
                    <Typography sx={{ fontSize: '10px', color: 'gray', textTransform: 'uppercase', fontWeight: 700 }}>Educators</Typography>
                    <Typography sx={{ fontSize: '16px', fontWeight: 800, color: educators >= nextTier.reqs.educators ? '#10B981' : COLORS.PRIMARY_NAVY }}>
                        {educators}/{nextTier.reqs.educators}
                    </Typography>
                </Box>
            )}
